test_that("MON-id: strong/weak monotonic increase - basic monotony tests", {
  expect_true(is_monotonic_along_ideal(c(1,2,3,4), rep(0,4), warn = FALSE))
  expect_false(is_monotonic_along_ideal(c(1,2,2,3), rep(0,4), warn = FALSE, strict = TRUE))
  expect_true(is_monotonic_along_ideal(c(1,2,2,3), rep(0,4), warn = FALSE, strict = FALSE))
})
...
test_that("MON-id: NA value", {
  expect_false(is_monotonic_along_ideal(c(1,2,NA,4,5,6), rep(0,6), warn = FALSE))
  expect_warning(is_monotonic_along_ideal(c(1,2,NA,4,5,6), rep(0,6), warn = TRUE))
})
...
test_that("MON-id: test compairson relative to ideal", {
  # base case
  expect_false(is_monotonic_along_ideal(c(1,2,3,4), rep(0,4), x_end = -4, warn = FALSE))
  ...
  for (i in 1:1000){
    ...
    any_fail <- any_fail | !is_monotonic_along_ideal(random_x_rot, random_y_rot, warn = FALSE)
  }
  expect_false(any_fail)

  # test different rotations of a sine wave as something that is monotonic, but swap last two x values
  x_template <- seq(0, 10*pi, length.out = 1000)
  y_template <- sin(x_template)
  ...
  for (i in 1:1000){
    ...
    random_x_rot <- random_x*cos(random_angle) - random_y*sin(random_angle)
    random_y_rot <- random_x*sin(random_angle) + random_y*cos(random_angle)
    all_fail <- all_fail & !is_monotonic_along_ideal(random_x_rot, random_y_rot,  warn = FALSE)
  }
